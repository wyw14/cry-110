...
func (a Accumulator) UsableVolume(minimumPressureBar float64) (float64, error) {
	...
	pressureFraction := (a.HeaderBar - minimumPressureBar) / (a.HeaderBar - a.PrechargeBar)
	prechargeHealth := math.Min(1, a.PrechargeBar/(minimumPressureBar*0.7))
	usable := math.Min(a.OilLiters, a.NominalLiters*pressureFraction*prechargeHealth)
	if usable < 0 {
		return 0, nil
	}
	return usable, nil
